...
const FacilityEdit = () => {
  ...
  useEffect(() => {
    api.get(`/facilities/${id}/`).then(res => setFormData(res.data)).catch(console.error);
  }, [id]);
  const handleChange = (e) => setFormData({ ...formData, [e.target.name]: e.target.value });
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.patch(`/facilities/${id}/`, formData);
      navigate('/facilities');
    } catch (error) {
  ...
  return (
    <Container maxWidth="md">
      <Paper sx={{ p: 4 }}>
        <Typography variant="h4" fontWeight={700} gutterBottom>Edit Facility</Typography>
        <form onSubmit={handleSubmit}>
          <Grid container spacing={3}>
            ...
            <Grid item xs={12}><Box display="flex" gap={2}><Button type="submit" variant="contained">Update Facility</Button><Button variant="outlined" onClick={() => navigate('/facilities')}>Cancel</Button></Box></Grid>
          </Grid>
        </form>
      </Paper>
    </Container>
