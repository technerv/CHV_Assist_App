...
const MemberEdit = () => {
  ...
  useEffect(() => {
    api.get(`/members/${id}/`).then(res => setFormData(res.data)).catch(console.error);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.patch(`/members/${id}/`, formData);
      navigate('/members');
    } catch (error) {
  ...
  return (
    <Container maxWidth="md">
      <Paper sx={{ p: 4 }}>
        <Typography variant="h4" fontWeight={700} gutterBottom>Edit Member</Typography>
        <form onSubmit={handleSubmit}>
          <Grid container spacing={3}>
            ...
            <Grid item xs={12}><Box display="flex" gap={2}><Button type="submit" variant="contained">Update Member</Button><Button variant="outlined" onClick={() => navigate('/members')}>Cancel</Button></Box></Grid>
          </Grid>
        </form>
      </Paper>
    </Container>
